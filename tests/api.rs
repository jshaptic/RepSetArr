...
#[tokio::test]
async fn the_kometa_text_format_serves_a_prefixed_id_per_line() {
    let state = state_from(CONFIG).await;
    let reply = get(&state, "/api/lists/everything?format=kometa-text").await;
    ...
    assert_eq!(
        reply.body.lines().skip(1).collect::<Vec<_>>(),
        vec!["tmdb:550", "tmdb:13", "tvdb:81189", "tmdb:1399"],
        "the TVDb-less show falls back to its TMDb id: {}",
        reply.body
    );
    assert!(
        reply.body.starts_with("# Generated by Repsetarr"),
        "{}",
        reply.body
    );
    assert_eq!(reply.header("X-Repsetarr-Count"), Some("4"));
}

#[tokio::test]
async fn the_kometa_json_format_serves_the_same_ids_as_a_list() {
    let state = state_from(CONFIG).await;
    let reply = get(&state, "/api/lists/everything?format=kometa-json").await;
    ...
    assert_eq!(
        ...
        json!([
            ...
            {"tmdb_id": 13},
            {"type": "tvdb", "id": 81189},
            {"type": "tmdb_show", "id": 1399}
        ])
    );
}

#[tokio::test]
async fn a_format_name_may_be_spelled_with_either_separator() {
    let state = state_from(CONFIG).await;
    let hyphen = get(&state, "/api/lists/everything?format=kometa-text").await;
    let underscore = get(&state, "/api/lists/everything?format=kometa_text").await;

    assert_eq!(underscore.status, StatusCode::OK);
    assert_eq!(underscore.body, hyphen.body);
}
...
async fn media_type_narrows_a_mixed_list() {
    ...
    let movies = get(
        &state,
        "/api/lists/everything?format=kometa-text&media_type=movies",
    )
    .await;
    assert_eq!(movies.status, StatusCode::OK);
    assert_eq!(
        movies.body.lines().skip(1).collect::<Vec<_>>(),
        vec!["tmdb:550", "tmdb:13"],
        "{}",
        movies.body
    );
    ...
    let shows = get(
        &state,
        "/api/lists/everything?format=kometa-text&media_type=shows",
    )
    .await;
    assert_eq!(
        shows.body.lines().skip(1).collect::<Vec<_>>(),
        vec!["tvdb:81189", "tmdb:1399"],
        "{}",
        shows.body
    );
...
async fn an_unknown_list_is_a_404() {
    let state = state_from(CONFIG).await;
    for format in ["json", "radarr", "sonarr", "kometa-text", "kometa-json"] {
...
async fn the_index_counts_movies_and_shows_per_list() {
    ...
    assert_eq!(
        everything["endpoints"]["kometa-text"],
        "/api/lists/everything?format=kometa-text"
    );
    assert_eq!(
        everything["endpoints"]["kometa-json"],
        "/api/lists/everything?format=kometa-json"
    );
}
